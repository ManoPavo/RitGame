...
func intro_animation():
	...
	tween.tween_property(logo, "scale", Vector2(0.5, 0.5), 0.25).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)
	tween.tween_interval(0.1)
	
	tween.tween_property(logo, "scale", Vector2(0.7, 0.7), 0.25)
	tween.tween_property(logo, "scale", Vector2(0.6, 0.6), 0.25)
